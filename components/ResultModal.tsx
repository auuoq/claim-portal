...
const ocrContractKeyLabels: Record<string, string> = {
...
export default function ResultModal({
  ...
  const [showOcrMarkdownFullScreen, setShowOcrMarkdownFullScreen] =
    useState(false);
  ...
  function getOcrResolvedKey(tab: string): string {
    ...
    const byKey = Object.keys(ocrData.ho_so_full).find(
      (k) => k.toLowerCase() === tabLower,
    );
    ...
    const found = Object.entries(ocrData.ho_so_full).find(
      ([_, arr]) =>
        expectedNorm && norm(arr?.[0]?.ten_giay_to ?? "") === expectedNorm,
    );
    return found ? found[0] : tab;
  }
  const ocrResolvedKey = ocrTab ? getOcrResolvedKey(ocrTab) : ocrTab;
  const ocrPages: HoSoPageEntry[] =
    useOcrFull && ocrResolvedKey
  ...
  const ocrActiveMarkdownLegacy = ocrContentArray
    .map((x: any) =>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fadeIn">
      ...
      <div
        className={`relative bg-white rounded-2xl shadow-2xl w-full max-h-[90vh] overflow-hidden animate-scaleIn flex flex-col ${mainTab === "ocr" && useOcrFull && currentOcrPage ? "max-w-6xl" : "max-w-4xl"}`}
      >
        {/* Header */}
        <div className="flex-shrink-0 bg-gradient-to-r from-teal-500 to-cyan-500 text-white px-6 py-4 flex items-center justify-between z-10">
        ...
        {/* Document Lists Panel - Only show if not in OCR tab or not using full OCR */}
        {mainTab !== "ocr" &&
          ((uploadedDocuments && uploadedDocuments.length > 0) ||
        ...
        {/* Content */}
        <div
          className={`flex-1 overflow-y-auto min-h-0 ${mainTab === "ocr" && !error ? "p-0" : "p-6"}`}
        >
          {error ? (
            // Error state
            <div className="py-6">
                  ...
                  <div className="mt-3">
                    <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4">
                      ...
                      {uploadedDocuments && uploadedDocuments.length > 0 ? (
                        ...
                        <p className="text-emerald-600 text-xs ml-2">
                          Chưa có loại tài liệu nào trong danh sách được upload.
                        </p>
                      )}
                    </div>
                  </div>
            ...
            mainTab === "analyse" ? (
              ...
              <div className="flex flex-col flex-1 min-h-0">
                {/* Contract Information - Top Area (Synced with OcrReviewPopup) */}
                ...
                {/* Document Lists - Only show Missing Documents in OCR tab */}
                {allMissingDocuments && allMissingDocuments.length > 0 && (
                  <div className="flex-shrink-0 px-6 py-4 border-b border-gray-200 bg-gray-50/50">
                    <div className="flex flex-wrap gap-6">
                      <div className="flex-1 min-w-[200px]">
                        ...
                        <ul className="flex flex-wrap gap-2">
                          {allMissingDocuments.map((doc, idx) => (
                            <li
                              key={idx}
                              className="inline-flex items-center gap-1.5 text-xs font-medium text-amber-700 bg-amber-50 border border-amber-200 rounded-full px-3 py-1"
                            >
                              <span className="w-1.5 h-1.5 rounded-full bg-amber-400 flex-shrink-0" />
                              {doc.ten}
                            </li>
                ...
                <div className="w-1/4 min-w-[260px] max-w-[320px] bg-gray-50 border-r border-gray-200 flex flex-col overflow-y-auto hide-scrollbar flex-shrink-0">
                  {ocrTabs.map((tab) => {
                    const documentName =
                      ...
                      (useOcrFull
                        ? ocrData?.ho_so_full?.[getOcrResolvedKey(tab)]?.[0]
                            ?.ten_giay_to
                        : ocrData?.ho_so?.[tab]?.[0]?.ten_giay_to) ??
                      tab.replace(/_/g, " ");
                    const count = useOcrFull
                      ? (ocrData?.ho_so_full?.[getOcrResolvedKey(tab)]
                          ?.length ?? 0)
                      : null;
                    return (
                      <button
                        ...
                        className={`
                          ...
                          ${ocrTab === tab ? "border-teal-500 text-teal-700 bg-white shadow-sm" : "border-transparent text-gray-600 hover:text-gray-900 hover:bg-gray-100"}
                        `}
                      >
                        <span className="truncate">{documentName}</span>
                        {count != null && (
                          <span
                            className={`flex-shrink-0 text-xs font-medium px-2 py-0.5 rounded-full ${count > 0 ? "bg-teal-100 text-teal-700" : "bg-gray-100 text-gray-500"}`}
                          >
                            {count}
                          </span>
                        )}
                ...
                <div className="flex-1 flex flex-col overflow-hidden min-w-0">
                  ...
                  {useOcrFull && ocrPages.length > 0 && (
                    <div className="flex-shrink-0 flex items-center justify-between gap-3 px-4 py-2.5 bg-gray-50 border-b border-gray-200 overflow-hidden">
                      <div className="flex items-center gap-2 overflow-hidden flex-nowrap">
                        <span className="text-sm font-medium text-gray-600 mr-1 flex-shrink-0">
                          Trang:
                        </span>
                        <div className="flex items-center gap-1 flex-shrink-0">
                          ...
                          {getPageRange().map((p, idx) => {
                            if (p === "...") {
                              return (
                                <span key={`dot-${idx}`} className="px-1 text-gray-400">
                                  ...
                                </span>
                              );
                            }
                        ...
                        {currentOcrPage && (
                          <span
                            className="text-xs text-gray-400 ml-1 truncate max-w-[120px] hidden sm:inline"
                            title={currentOcrPage.file_name}
                          >
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        {canShowImage && (
                          <button
                            ...
                            onClick={() => {
                              setImageError(false);
                              setShowOcrImageModal(true);
                            }}
                            className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-gray-300 bg-white text-gray-700 hover:bg-gray-50 text-sm font-medium"
                          >
                  ...
                  <div className="flex-1 p-6 overflow-y-auto bg-white flex flex-col gap-6 min-h-0">
                    {ocrActiveMarkdown ? (
                      <div className="prose prose-teal max-w-none">
                        <ReactMarkdown
                          remarkPlugins={[remarkGfm]}
                          components={ocrMarkdownComponents}
                        >
                          {ocrActiveMarkdown}
                        </ReactMarkdown>
                      </div>
                      ...
                      <div className="text-center text-gray-500 py-10 px-4 bg-gray-50 rounded-xl border border-dashed border-gray-200">
                        <p className="font-medium">
                          Chưa có nội dung OCR cho loại giấy tờ này.
                        </p>
                        <p className="text-sm mt-1">
                          Dữ liệu có thể đang được xử lý hoặc tài liệu không có
                          trang nào được phân loại.
                        </p>
                      </div>
                    )}
                  </div>
              ...
              >
                ...
                {imageError ? (
                  <div className="bg-white rounded-xl p-8 text-center text-gray-600">
                    ...
                    <p className="text-sm">
                      Trang không tồn tại hoặc phiên đã hết hạn (30 phút).
                    </p>
                  </div>
                ) : (
        ...
        {showOcrMarkdownFullScreen && (
          ...
          >
            {/* Header */}
            <div className="flex-shrink-0 flex items-center justify-between px-6 py-4 bg-teal-600 text-white shadow-md">
              <span className="font-semibold flex items-center gap-2">
                <FontAwesomeIcon icon={faExpand} className="w-4 h-4" />
                Nội dung bóc tách —{" "}
                {currentOcrPage
                  ? `${currentOcrPage.file_name} (Trang ${currentOcrPage.page_number})`
                  : "OCR"}
              </span>
              <button
                type="button"
            ...
            {/* Pagination for Full Screen */}
            {useOcrFull && ocrPages.length > 0 && (
              <div className="flex-shrink-0 flex items-center justify-center gap-2 px-4 py-3 bg-gray-50 border-b border-gray-200">
                <div className="flex items-center gap-1.5 overflow-x-auto hide-scrollbar max-w-full">
                  ...
                  {ocrPages.map((_, i) => (
                    <button
                      ...
                      type="button"
                      onClick={() => setOcrPageIndex(i)}
                      className={`min-w-[2.5rem] h-9 px-3 rounded-lg text-sm font-semibold transition-all ${i === ocrPageIndex ? "bg-teal-600 text-white shadow-md" : "text-gray-600 hover:bg-gray-200 bg-white border border-gray-200"}`}
                    >
                      {i + 1}
                    </button>
                  ))}
            ...
            <div className="flex-1 flex flex-col lg:flex-row overflow-hidden bg-gray-100">
              {/* Left: Image Viewer */}
              <div className="flex-1 overflow-auto bg-gray-200 flex items-center justify-center p-4 border-b lg:border-b-0 lg:border-r border-gray-300">
                {imageError ? (
                  <div className="bg-white rounded-xl p-8 text-center text-gray-600 shadow-sm border border-gray-200">
                    ...
                    <p className="font-medium">Không tải được ảnh trang</p>
                    <p className="text-sm">
                      Trang không tồn tại hoặc phiên đã hết hạn.
                    </p>
                  </div>
                ) : (
                  <div className="relative group">
                    <img
                      src={ocrImageUrl}
                      alt={`Trang ${currentOcrPage?.page_number ?? 1}`}
                ...
                <div className="max-w-3xl mx-auto">
                  {ocrActiveMarkdown ? (
                    ...
                    <div className="text-center py-20 text-gray-400 italic">
                      Không có nội dung bóc tách cho trang này.
                    </div>
                  )}
                </div>
              </div>
